...
class EventChain {
 ...
 public:
  ...
  void schedule(Event *e) {
    ...
    Event **ppcur = &head;
    while ((*ppcur != NULL) && ((*ppcur)->sched_time <= e->sched_time)) ppcur = &((*ppcur)->next);
...
void Sender_StopTimer() {
  if (tracing_level >= 1) fprintf(stdout, "Time %.2fs (Sender): the timer is stopped.\n", sim_core.time());
...
int main(int argc, char *argv[]) {
  ...
  fprintf(stdout,
          ...
          sim_time, msg_arrivalint, msg_size, outoforder_rate * 100.0, loss_rate * 100.0, corrupt_rate * 100.0,
          tracing_level);
  fgetc(stdin);
    ...
    switch (e->event_type) {
      case EVENT_SENDER_FROMUPPERLAYER: {
        ...
        if (sim_core.time() < sim_time) {
          real_e->sched_time = sim_core.time() + msg_arrivalint * 2.0 * myrandom();
